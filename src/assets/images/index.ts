// src/assets/images/index.ts
import apartments from "./apartments.jpg";
import bull from "./bull.jpg";
import cows from "./cows.jpg";
import diamondRing from "./diamond-ring.jpg";
import donation from "./donation.jpg";
import f1car from "./f1-car.jpg";
import familyHome from "./family-home.jpg";
import ferrari from "./ferrari.jpg";
import fordF150 from "./ford-f-150.jpg";
import goldBar from "./gold-bar.jpg";
import helicopter from "./helicopter.jpg";
import highEndRestaurant from "./high-end-restaurant.jpg";
import horses from "./horses.jpg";
import hulu from "./hulu.png";
import iphone17 from "./iphone17.jpg";
import land from "./land.jpg";
import luxuryWine from "./luxury-wine.jpg"; 
import macbook from "./macbook.jpg";
import mansion from "./mansion.jpg";
import monsterTruck from "./monster-truck.jpg"; 
import movie from "./movie.webp"; 
import netflix from "./netflix.jpg";
import privateJet from "./private-jet.jpg";
import resortCenter from "./resort-center.jpg";
import rolex from "./rolex.jpg";
import samsung from "./samsung.jpg";
import ship from "./ship.jpg";
import spotify from "./spotify.jpg";
import supermarket from "./supermarkets.jpg";
import tesla from "./tesla.jpg";
import vacation from "./vacation.jpg";
import yacht from "./yacht.jpg";
import youtube from "./youtube.jpg";

export {
  apartments,
  bull,
  cows,
  diamondRing,
  donation,
  f1car,
  familyHome,
  ferrari,
  fordF150,
  goldBar,
  helicopter,
  highEndRestaurant,
  horses,
  hulu,
  iphone17,
  land,
  luxuryWine,
  macbook,
  mansion,
  monsterTruck,
  movie,
  netflix,
  privateJet,
  resortCenter,
  rolex,
  samsung,
  ship,
  spotify,
  supermarket,
  tesla,
  vacation,
  yacht,
  youtube
};
